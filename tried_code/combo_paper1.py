...
REBUILD_DATA = True
#IMG_SIZE = 200 #make images 50*50. To make the input uniform.

data_dim = 1            #keep 3 for 3D (RGB) data and 1 for grayscale data
input_width = 200
input_height = 200
output_size = 2
"""
...
X = torch.Tensor([i[0] for i in training_data]).view(-1,input_width,input_height)
X = X/255.0
...
for epoch in range(EPOCHS):
    for i in tqdm(range(0, len(train_X), BATCH_SIZE)): # from 0, to the len of x, stepping BATCH_SIZE at a time. [:50] ..for now just to dev
        ...
        batch_X = train_X[i:i+BATCH_SIZE].view(-1, 1, input_width, input_height)
...
with torch.no_grad():
    for i in tqdm(range(len(test_X))):
        ...
        net_out = net(test_X[i].view(-1, 1, input_width, input_height))[0]  # returns a list, 
